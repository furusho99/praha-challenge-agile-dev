import { boolean, integer, pgTable, serial, text, timestamp, varchar, primaryKey, foreignKey } from "drizzle-orm/pg-core";
...
export const usersTable = pgTable("users", {
  ...
  status: varchar('status', { length: 50 }).references(() => usersStatusTable.status),
...
export const usersAssignmentsTable = pgTable("users_assignments", {
  usersId: integer("users_id").notNull().references(() => usersTable.id),
  assignmentsId: integer("assignments_id").notNull().references(() => assignmentsTable.id),
